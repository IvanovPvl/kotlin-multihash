...
enum class HashType(val index: Int, val length: Int) {
    id(0, -1),
    md5(0xd5, 16),
    sha1(0x11, 20),
    sha2_256(0x12, 32),
...
data class MultiHash(val hashType: HashType, val hash: ByteArray) {
    companion object {
        private const val MAX_IDENTITY_HASH_LENGTH = 1024*1024

        private val lookup = mapOf(*HashType.values().map { it.index to it }.toTypedArray())

        fun hashWithSHA256(data: ByteArray): MultiHash {
            val digest = MessageDigest.getInstance("SHA-256")
            return fromHash(HashType.sha2_256, digest.digest(data)).component1()!!
        }

        fun hashWithSHA512(data: ByteArray): MultiHash {
            val digest = MessageDigest.getInstance("SHA-512")
            return fromHash(HashType.sha2_512, digest.digest(data)).component1()!!
        }

        private fun fromHash(hashType: HashType, hash: ByteArray): Result<MultiHash, IllegalStateException> {
            if ((hash.size > 127 && hashType != HashType.id) || (hash.size > MAX_IDENTITY_HASH_LENGTH)) {
                return Result.error(IllegalStateException("Unsupported hash size : ${hash.size}"))
            }

            if (hash.size != hashType.length && hashType != HashType.id) {
                return Result.error(IllegalStateException("Incorrect hash size : ${hash.size} != ${hashType.length}"))
            }

            return Result.success(MultiHash(hashType, hash))
        }
    }
}
